...
func TestForm_MinLength(t *testing.T) {
	...
	form.MinLength("x", 10)
	if form.Valid() {
		t.Error("form shows min length for a field that does not exist")
	}
	...
	if form.Valid() {
		t.Error("shows minlength is reached when data is shorter")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	if !form.Valid() {
		t.Error("got an invalid email error when we should not have")
	}
	...
	if form.Valid() {
		t.Error("got valid email when we should not have")
	}
}
